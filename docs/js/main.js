window.addEventListener('DOMContentLoaded', async(event) => {
    ...
    document.getElementById('get-address').addEventListener('click', async(event) => { init() });
    async function init(address=null) {
        if (window.hasOwnProperty('mpurse')) {
            ...
            navigator.clipboard.writeText(document.getElementById('address').value).then(()=>{Toaster.toast('アドレスをコピーしました')}, ()=>{Toaster.toast('アドレスのコピーに失敗しました……', true)})
        }
    }
